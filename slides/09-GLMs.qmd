...
## Toxtest mit dem `drc`-Paket

<br>

- Das $\text{drc}$-Paket bietet Funktionen speziell für die Dosis-Wirkungs-Analyse.
- Daten **nicht** im $\text{cbind}$-Format sondern Verhältnis (events/trials) und Gesamtzahl (`weights`).
- Falls die Dosis im logarithmierten Format vorliegt, muss die Basis angegeben werden (`logDose`).
...
## Plot und $\text{ED}_{50}$ ($\text{LC}_{50}$)

<br>

`drc` vereinfacht sowohl die Visualisierung als auch die Schätzung der $\text{EC}_{50}$.

:::{.columns}

::::{.column width="50%"}

```{r drc-code-lc50, eval=TRUE, echo=TRUE, fig.show='hide'}
# Plot mit 95% CI
...
::::{.column width="50%"}
```{r drc-plot-lc50, eval=TRUE, echo=FALSE}
plot(m_drc, type = "all", xlab = "Log2-Dosis")
```
::::
:::
...
- Die Schätzung der EC~50~ und des CI ist hier direkter und robuster als mit manuellen Methoden.
...
* **GLM** (Logistische Regression): fundamentale Grundlage zur Modellierung von Proportionen. 
    * Es schätzt zwei Parameter (EC~50~ und Steigung).
    * Relativ starr, erzwingt eine Symmetrie um den EC~50~.

* **Dose Response Curves** mit `drc`
    * **flexiblere Log-Logistischee Modelle, z.B. (LL.3)** mit drei Parametern<br> (Steigung, EC~50~, Asymptote). 
