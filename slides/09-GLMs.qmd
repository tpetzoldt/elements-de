...
```
- Durch die Funktion `I()` (AsIs) wird der Term arithmetisch und nicht symbolisch interpretiert.
...
- **Test:** Der **Chi-Quadrat-Test** aus der ANOVA zeigt, ob das Modell mit `dose_log` besser ist als ein Nullmodell.
...
```{r ec50-calc, echo=TRUE}
# Koeffizienten und Varkov-Matrix abrufen
b <- coef(m); V <- vcov(m)
...
```{r ec50-ci-pure-r, echo=TRUE}
se <- sqrt(V[1,1]/b1^2 + V[2, 2] * b0^2 / b1^4 - 2*V[1,2] * b0/b1^3)

# 95%-Konfidenzintervall
ec50 <- -b0 / b1
CI <- ec50 + qnorm(c(0.025, 0.975)) * se
cat("ec50 =", ec50, "CI = (", CI, ")\n")
```
<!----------------------------------------------------------------------------->
...
- Das $\text{drc}$-Paket [@Ritz2015-drc] enthält Funktionen speziell für die Dosis-Wirkungs-Analyse.
- Daten **nicht** im $\text{cbind}$-Format, sondern Verhältnis (events/trials) und Gesamtzahl (`weights`).
- Falls die Dosis im logarithmiert vorliegt, muss die Basis angegeben werden (`logDose`).

```{r drc-fit, eval=TRUE, echo=TRUE}
library(drc)
...
summary(m_drc)
```
* Das 3-Parameter-Modell (LL.3) ist oft eine robustere Alternative zum einfachen $\text{GLM}$.

## Plot und ED~50~ bzw. EC~50~

<br>

**drc*+ vereinfacht Visualisierung und Schätzung der EC~50~.
...
```{r drc-code-ec50, eval=TRUE, echo=TRUE, fig.show='hide'}

plot(m_drc, type = "all", xlab = "Log2-Dosis")
```

* Schätzung der EC50 (ED50) mit CI

```{r drc-code-vis, eval=TRUE, echo=TRUE, fig.show='hide'}
ED(m_drc, respLev = 50, interval = "delta")
```
::::

::::{.column width="60%"}
```{r drc-plot-ec50, eval=TRUE, echo=FALSE}
par(cex=1.4)
plot(m_drc, type = "all", xlab = "Log2-Dosis")
plot(m_drc, type = "bar", add=TRUE)
```
::::
:::
...
* Schätzung von EC~50~ und des CI ist mit **drc** direkter und robuster als mit `glm`.
* CI an den Rändern nur approximativ (überschreitet Intervall (0, 1))
...
* **GLM** (Logistische Regression): fundamentale Grundlage zur Modellierung von Proportionen. 
    * Schätzt zwei Parameter (EC~50~ und Steigung)
    * Relativ starr, erzwingt eine Symmetrie um den EC~50~

* **Dose Response Curves** mit `drc`
    * **flexiblere log-logistische Modelle, z.B. (LL.3)** mit drei Parametern<br> (Steigung, EC~50~, Asymptote). 
    * präzisere Anpassung an biologische Dosis-Wirkungs-Daten.

**Modellvergleich:** $\rightarrow$ Füllen Sie folgende Tabelle aus:

<br>

| Modell              | EC~50~               | $CI$ (lower, upper) |
| :---                | :---                 | :---                |
| GLM (Linear Logit)  | ......               | ......, ......      |
| drc (`LL.3`)        | ......               | ......, ......      |
...
Das Modell nimmt an, dass sich sowohl die EC~50~ als auch die **Steigung** der Dosis-Wirkungs-Kurve zwischen den Substanzen **unterscheiden**.
<!----------------------------------------------------------------------------->

## Binomiale ANCOVA: Mehrere Substanzen gleichzeitig

**Fragestellung:** Sind die EC~50~ von 2 Substanzen signifikant verschieden?


```{r daphnia2-data, echo=TRUE}
daphnia2 <- data.frame(
  dead = c(0, 0, 1, 3, 7, 9, 10, 
           1, 1, 3, 6, 9, 10, 10),
  total = 10,
  dose_log = rep(1:7, 2),
  substance = factor(rep(c("A", "B"), each = 7))
)
```
...
- Im folgenden verwenden wir das Paket **tinyplot** [@McDermott2025-tinyplot] sowie Pakete aus der **easystats**-Familie [@Luedecke2022-easystats;@Luedecke2021-performance]. 
...
```{r, echo=TRUE, fig.width=16, fig.height=8, fig.align='center'}
library(performance)
...
```{r daphia2-parameters, echo=TRUE, eval=TRUE}
library(parameters)
mp <- model_parameters(m_AB, exponentiate=FALSE)
mp
```
## Schätzung der EC50
...
cat("EC50 (Conc) A, B:", round(ec50_conc_A, 1), round(ec50_conc_B, 1), "\n")
```
...
* Die **Dosis-Wirkungs-Analyse mit dem R-Paket drc** wird umfassend im Buch von @Ritz2019DoseResponse behandelt. Es enthält die Grundlagen und fortgeschrittene Techniken zur Modellierung nichtlinearer Kurven.

* Das Buch von @Crawley2012RBook enthält neben klassischen statistischen Tests und ANOVA auch eine Reihe von Beispielen zur Anwendung von GLMs.

* Das Buch von @Zuur2009MixedModels zur Anwendung gemischter Modelle (Mixed Models) enthält u.a. auch Beispiele für GLMs, speziell ausgerichtet auf ökologische Fragestellungen.
