...
```{r uniform-cdf, fig.width=4, fig.height=4}
```
[Kumulative Verteilungsfunktion]{.blue}
:::
...
```{r boxcox, echo = TRUE, fig.width=16, fig.height=5, fig.align='center'}
library(MASS)
dat <- read.csv("https://tpetzoldt.github.io/datasets/data/prk_nit.csv")
Nit90 <- dat$biovol[dat$group == "nit90"]
boxcox(Nit90 ~ 1)
...
```{r boxcox1, echo = TRUE, fig.width=10, fig.height=5, fig.align='center'}
boxcox(Nit90 ~ 1)
```
...
```{r boxcox3, echo=TRUE, fig.width=10, fig.height=6, fig.align='center'}
dat <- read.csv("https://tpetzoldt.github.io/datasets/data/prk_nit.csv")
...
Datenquelle: Bundesanstalt für Gewässerkunde (BFG), siehe [Nutzungshinweise](https://github.com/tpetzoldt/datasets/blob/main/data/elbe_info.txt).

* **Links:** große Anzahl von Extremwerten, sind das Ausreißer?
* **Mitte:** Verteilung ist rechtsschief.
* **Rechts:** Transformation (3-parametrische Lognormalverteilung) <br>
