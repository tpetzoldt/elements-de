...
## Kontinuierliche Gleichmäßigkeitsverteilung $\mathbf{U}(0, 1)$
...
* **Binning:** Unterteilung der Werte in Klassen
...
::: {.column width="59%"}
Die **cdf** ist das Integral der Dichtefunktion:
...
Bei der Gleichverteilung ist es so:
...
* von großer theoretischer Bedeutung aufgrund des zentralen Grenzwertsatzes (CLT)
* ergibt sich aus der Addition einer großen Anzahl von Zufallswerten gleicher Größenordnung.

**Die Dichtefunktion der Normalverteilung ist mathematisch schön.**
...
> sind normalverteilt, unabhängig vom Typ der
...
1. Erstelle eine Matrix mit 100 Zeilen und 25 Spalten von gleichmäßig verteilten Zufallszahlen
...
## Dichte und Quantile der Standardnormalen
...
## Kumulative Verteilungsfunktion -- Quantilsfunktion
...
## Standardnormale, Skalierung und Verschiebung
...
Jede Normalverteilung kann skaliert verschoben werden, um eine Standardnormalverteilung mit $\mu=0, \sigma=1$ zu bilden.
...
* konvergiert zur Normalverteilung für $df \rightarrow \infty$
...
## Übergeordnete Verteilung der Lognormalverteilung
...
* log von Werten einer Lognormalverteilung $\rightarrow$ übergeordnete Normalverteilung.
...
* Wie viele „6“ mit Wahrscheinlichkeit $1/6$ in 3 Versuchen?
* Medizin, Toxikologie, Vergleich von Prozentzahlen
* Ähnlich, aber ohne Ersetzen: Hypergeometrische Verteilung im Lotto
...
* Mittelwert und Varianz sind gleich ($\mu = \sigma^2$), daraus ergibt sich der Parameter „lamda“ ($\lambda$) 
...
**Typischer Fehler der Zellzählung:** 95% Konfidenzintervall
...
$\rightarrow$ Ziel: Prüfung, ob eine Stichprobe einer Normalverteilung entspricht
...
[Empfehlung: Verwende grafische Überprüfungen. Vertraue nicht auf Shapiro Wilks!]{.red}
...
* Moderne Methoden können bestimmte Verteilungen direkt verarbeiten, z. B. Binomial-, Gamma- oder Poisson-Verteilungen.
...
* $x'=a - 1/\sqrt{x}$ [(um die Skala einfacher zu gestalten)]{.gray}
...
## Zur Erinnerung: Der zentrale Grenzwertsatz (CLT)
...
* Wir können Methoden verwenden, die von einer Normalverteilung für nicht-normale Daten ausgehen.
...
* Die Variabilität des Mittelwerts halbiert sich, wenn wir den Stichprobenumfang viermal erhöhen ($2^2$).
...
* Schätze anhand von $\bar{x}$ und $s_\bar{x}$ das Intervall, in dem $\mu$ mit einer bestimmten Wahrscheinlichkeit gefunden wird
* weniger abhängig von der ursprünglichen Verteilung der Daten aufgrund des CLT
...
* kann auch für $n>30$ verwendet werden
...
* Extrem große oder extrem kleine Werte werden manchmal als „Ausreißer“ bezeichnet. 
