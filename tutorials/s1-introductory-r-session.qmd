...
Ein gutes Beispiel für den Anfang ist das kurze Video „R tutorial - The True Basics of R“ von DataCamp, siehe [https://youtu.be/SWxoJqTqo08](https://youtu.be/SWxoJqTqo08). Außerdem bietet [Datacamp](https://www.datacamp.com/) ausgezeichnete **R**-Tutorials, die zum Teil kostenlos sind. Ein weiteres nützliches **R**-Tutorial gibt es bei [W3Schools.com](https://www.w3schools.com/r/).

# Erste Schritte

## Programmstart und Hilfesystem

Der einfachste Weg, **R** zu erlernen, ist das kreative Verstehen und Abwandeln von gegebenen Beispielen, die Verwendung von **R** zur Lösung praktischer Probleme und die Diagnose der häufig auftretenden Probleme und Fehlermeldungen. Keine Sorge: Fehlermeldungen sind ein normales Phänomen im wissenschaftlichen Rechnen und kein Hinweis auf eine Funktionsstörung des Computers oder des menschlichen Gehirns. Das Gegenteil ist der Fall: Ein gewisses Maß an Stresshormonen hilft, dauerhafte Lerneffekte zu erzielen. Nach einer gewissen Erfahrung ist dann die offizielle **R**-Dokumentation „An Introduction to R“ [@Venables2021]. oder ein gutes **R**-Buch sehr zu empfehlen.
...
Wir beginnen unsere erste Sitzung mit dem Start von **RStudio**, einer plattformunabhängigen Schnittstelle, die das Arbeiten mit R erleichtert. RStudio unterteilt den Bildschirm in 3 (bzw. 4) Fenster (sogenannte Panes), von denen einige zusätzliche Tabs haben, um zwischen verschiedenen Ansichten zu wechseln.

![](../img/rstudio.png)

**Abbildung 1:** R Studio mit 4 Fenstern. Benutze **Datei -- Neues R-Skript**, um das Quellcode-Fenster zu öffnen (siehe oben links). Gib dann etwas Code ein und vergiss nicht, die Hilfsdateien zu lesen.
...
Einen arithmetischen Ausdruck wie diesen eingeben:

```{r pocketcalc, eval=FALSE, fig=FALSE, echo=TRUE, keep.source=TRUE}
2 + 4
```

zeigt, dass **R** als Taschenrechner verwendet werden kann, der das Ergebnis sofort ausgibt:

```{r pocketcalc, eval=TRUE, echo=FALSE}
```

Anstatt das Ergebnis auf dem Bildschirm auszugeben, ist es auch möglich, das Ergebnis mit dem Zuweisungsoperator „`<-`“ in einer benannten **Variablen** zu speichern.
...
Vielleicht hast du bemerkt, dass die Ausgabe des obigen Beispiels ein führendes `[1]` hatte, was bedeutet, dass die Zeile mit dem ersten Element von `a` beginnt.  Dies bringt uns zu einer sehr wichtigen Eigenschaft von **R**, dass Variablen mehr als nur einzelne Werte enthalten können: Vektoren, Matrizen, Listen, Datenrahmen (Tabellen) und so weiter.
...
Sequenzen von wiederholten gleichen Werten können mit `rep` erhalten werden:
...
Anstatt auf Vektoren als Ganzes zuzugreifen, ist es auch möglich, einzelne Elemente zu extrahieren, wobei der Index der angeforderten Daten selbst ein Vektor ist:
...
Manchmal haben die Elemente eines Vektors individuelle Namen, was den Zugriff auf sie erleichtert:
...
Es ist auch möglich, einen gegebenen Vektor durch Voranstellen oder Anhängen von Werten mit der Kombinationsfunktion (`c`) zu erweitern:

```{r fig=FALSE, echo=TRUE}
c(1, 1, values, 0, 0)
```

Die Länge eines Vektors kann bestimmt werden mit:

```{r fig=FALSE, echo=TRUE}
length(values)
```

und es ist auch möglich, leere Vektoren zu haben, d. h. Vektoren, die zwar existieren, aber keine Werte enthalten. Hier bedeutet das Schlüsselwort `NULL` „nichts“ im Gegensatz zu „0“ (Null), was die Länge 1 hat:
...
Soll ein Datenelement vollständig entfernt werden, so kann dies mit der Funktion `remove` geschehen:
...
als guten Kompromiss zwischen Glättung und Speicherbedarf, also setzen wir die Schrittweite auf 0.1:
...
Jetzt wollen wir eine Kosinusfunktion mit einer anderen Farbe hinzufügen. Dies kann mit einer der Funktionen `lines` oder `points` gemacht werden, um Linien oder Punkte zu einer bestehenden Figur hinzuzufügen:
...
Es gibt viele Optionen, um das Verhalten der meisten Grafikfunktionen zu ändern. Im Folgenden werden benutzerdefinierte Koordinatengrenzen (`xlim`, `ylim`), Achsenbeschriftungen und eine Überschrift (`xlab`, `ylab`, `main`) angegeben.
...
Das obige Beispiel zeigt einen ziemlich langen Befehl, der möglicherweise nicht in eine einzige Zeile passt. In solchen Fällen zeigt **R** ein `+` (Pluszeichen) an, um anzuzeigen, dass ein Befehl fortgesetzt werden muss, z.B. weil noch eine schließende Klammer oder ein schließendes Anführungszeichen fehlt. Ein solches `+` am Anfang einer Zeile ist ein automatischer „Prompt“ ähnlich dem gewöhnlichen `>`-Prompt und muss niemals manuell eingegeben werden. Wenn die „+“-Fortsetzungsaufforderung jedoch versehentlich auftritt, drücke „ESC“, um diesen Modus abzubrechen.

Im Gegensatz zur langzeiligen Fortsetzungsaufforderung ist es auch möglich, mehrere Befehle in eine Zeile zu schreiben, getrennt durch ein Semikolon „`;`“. Dies ist in einigen Fällen sinnvoll, aber im Allgemeinen ist es viel besser, den Skript-Editor zu verwenden und dann:
...
Nun fügen wir ein sogenanntes *Normalwahrscheinlichkeitsdiagramm* und ein zweites Histogramm mit relativen Häufigkeiten zusammen mit der glockenförmigen Dichtekurve der Standardnormalverteilung. Das optionale Argument `probability = TRUE` sorgt dafür, dass das Histogramm die gleiche Skalierung wie die Dichtefunktion hat, so dass beide überlagert werden können:
...
Hier bietet sich auch eine gute Gelegenheit, eine kleine zusammenfassende Statistik zu erstellen: z.B. `mean(x)`, `var(x)`,
`sd(x)`, `range(x)`, `summary(x)`,
`min(x)`, `max(x)`, ...

Oder wir können prüfen, ob die generierten Zufallszahlen `x` annähernd normalverteilt sind, indem wir den Shapiro-Wilks-W-Test verwenden:
...
Neben Vektoren enthält **R** noch einige andere Objektklassen zur Speicherung von Daten, z.B. `matrix`, `array`, `list` oder `data.frame`. Sowohl „base **R**“ als auch beitragende Pakete enthalten viele weitere Klassen. Es ist auch möglich, eigene benutzerdefinierte Klassen zu definieren.
...
Modi können „numerisch“ für Berechnungen oder „Zeichen“ für Textelemente sein. 
...
Das Folgende ist ebenfalls eine Zeichenvariable, da die Zahlen in Anführungszeichen stehen. Es ist dann nicht möglich, Berechnungen durchzuführen:
...
die wie Zeichen aussieht, z. B. „Kontrolle“, „Behandlung A“, „Behandlung B“ ..., aber ihre **Stufen** sind intern als Ganzzahl kodiert. 
...
Wir sehen, dass die Zeichenvariable mit Anführungszeichen gedruckt wird und der Faktor ohne Anführungszeichen, aber mit einer zusätzlichen Information über die **Stufen**. Der Grund dafür ist, dass der Faktor intern als Ganzzahlwerte mit zugewiesenen Stufen als Übersetzungstabelle kodiert ist:
...
Faktoren sind nützlich für statistische Analysen wie ANOVA und statistische Tests und auch als Kategorien für die Darstellung:
...
Bei der Rückkonvertierung solcher Faktoren in numerische Variablentypen ist jedoch Vorsicht geboten, da ein Zeichen „123“ mit einem anderen Wert (z. B. 1) und nicht mit 123 kodiert werden kann, siehe die folgende Demonstration einer korrekten und einer falschen Faktorumwandlung:
...
Eine solche Faktorkodierung ist nicht spezifisch für **R** und kommt auch in anderen Statistikpaketen vor. Sie werden dann manchmal als „Dummy-Variablen“ bezeichnet.

## Matrizen und Arrays

Eine Matrix ist eine zweidimensionale Datenstruktur, die für Matrixalgebra verwendet werden kann. Um eine Matrix zu erstellen, können wir zunächst einen eindimensionalen Vektor erstellen und ihn dann als zweidimensionale Matrix mit „nrow“-Zeilen und „ncol“-Spalten umformatieren:
...
Wir sehen, dass die Matrix zeilenweise gefüllt ist. Wir können sie auch wieder in einen Vektor umwandeln:
...
Vektoren, Matrizen und Arrays haben eine wichtige Einschränkung: Sie können nur einen Datentyp (Modus) enthalten, entweder einen numerischen oder einen Zeichentyp. Wenn also ein einzelnes Element vom Typ Zeichen ist, hat die gesamte Matrix den Modus Zeichen und erscheint in Anführungszeichen:
...
Die typische Datenstruktur für die Datenanalyse in **R** ist der sogenannte `data.frame`. Er kann sowohl Spalten mit numerischen Daten als auch Spalten mit Moduszeichen enthalten. Einige Pakete verwenden erweiterte Versionen von Datenrahmen, so genannte `tibble`.

Ein Daten Frame kann von Grund auf direkt im **R**-Code erstellt werden erstellt oder aus einer Datei oder dem Internet gelesen werden. Als Beispiel wurden die Studierenden in verschiedenen Jahren nach ihrer Lieblingszahl von eins bis 9 gefragt. Die Ergebnisse können wie folgt in einen Daten Frame eingetragen werden:
...
2. Eingabe aus einer Textdatei
3. Eingabe aus einer Tabellenkalkulationsdatei (LibreOffice oder Excel)
4. Eingabe aus einer Textdatei aus dem Internet
5. Eingabe aus einer Datenbank


Darüber hinaus ist es möglich, Daten aus anderen Statistikpaketen wie SPSS, SAS oder Stata zu importieren (`library(foreign)`), GIS-Shapefiles zu lesen (`library(shapefiles)`), und sogar Sounddateien oder Bilder einzulesen.
...
Auf die gleiche Weise ist es möglich, andere Datentypen wie Daten Frames zu erstellen:
...
Wie wir sehen, unterscheidet sich eine Matrix nicht wesentlich von einem Vektor, der in Zeilen und Spalten formatiert ist.
...
Die Daten können heruntergeladen werden von [https://github.com/tpetzoldt/datasets/tree/main/data](https://github.com/tpetzoldt/datasets/tree/main/data)
...
Da die Daten im Daten Frame lakes` gespeichert sind, ist es nun möglich, wie gewohnt auf sie zuzugreifen:
...
Hinweis: Im obigen Beispiel ist der Name des Daten Frames identisch mit dem Dateinamen, d.h „lakes“. Wenn wir ihn anders benennen wollen (z.B.: `dat`), dürfen wir nicht vergessen, diese Einstellung zu ändern.

Beachte auch, dass die Code-Vorschau die Befehle enthält, die der Assistent erstellt hat. Wenn wir diese Befehle in das Skriptfenster kopieren, können die Daten mehrmals gelesen werden ohne zum Menüsystem zurückkehren zu müssen:
...
## Den Inhalt des Datenframes anzeigen

Am einfachsten ist es, einfach den Namen des Datenframes in die **R**-Konsole einzugeben, z.B.:

```{r}
lakes
```

oder auf den Namen des Datenframes im „Environment“-Explorer von **RStudio** zu klicken.
Dies führt dann `View(lakes)` aus, so dass die Daten angezeigt werden.

Bei großen Tabellen ist es oft nicht sehr sinnvoll, den gesamten Inhalt mit `View` anzuzeigen, so dass es besser sein kann, die Funktion `str` (Struktur) zu verwenden, die einen kompakten Überblick über Typ, Größe und Inhalt einer Variablen gibt:
...
Auf einzelne Spalten eines Datenrahmens kann durch die Verwendung von Indizes (mit `[]`) ähnlich wie bei einem Vektor oder einer Matrix oder durch die Verwendung des Spaltennamens und des Operators `$`) zugegriffen werden:
...
Wir sollten auch den feinen Unterschied zwischen der Ausgabe der `[]` und der `[[]]`- Version beachten.  Der Unterschied ist folgender: einfache Klammern geben einen Datenframe mit einer Spalte zurück, aber doppelte eckige Klammern geben den Inhalt der Spalte als Vektor ohne Beschriftung zurück.
...
Stattdessen ist es viel besser, eine andere Funktion `width` zu verwenden, die den Datenframe nur vorübergehend öffnet:
...
Eine Teilmenge eines Datenframes kann auch mit der Funktion `subset` extrahiert werden:
...
Wir können auch auf einzelne Elemente in Matrix-ähnlicher Weise zugreifen.

Das Element aus der 2. Zeile und der 4. Spalte kann ausgewählt werden mit:
...
Weitere Methoden für die Arbeit mit Matrizen, Datenframes und Listen sind in **R**-Lehrbüchern oder in der offiziellen **R**-Dokumentation zu finden.
...
Dies funktioniert gut und wird immer noch verwendet, aber die modernen Methoden sind kompakter und einfacher zu verstehen. Hier wollen wir zunächst ein modernes Konzept vorstellen, das „Pipelining“ genannt wird. Die Idee ist, dass das Ergebnis einer Funktion direkt an eine andere Funktion weitergeleitet wird, also anstatt zu schreiben:
...
Die obige Zeile `summarise` kann immer noch besser gemacht werden, und es gibt viele andere verblüffende Möglichkeiten, daher werden wir später darauf zurückkommen.
...
Nehmen wir das obige Beispiel und speichern wir die Ergebnisse in einem neuen Datenrahmen `results`:
...
Datenframes können als Textdateien mit `write.table`, `write.csv` oder `write_csv` gespeichert werden. Hier verwenden wir `write_csv` (mit Unterstrich, nicht Punkt) aus dem Paket **readr**:
...
**R** enthält viele Datensätze zum Erforschen seiner grafischen und statistischen Funktionen, die mit der data-Funktion aktiviert werden können, z.B. `data(iris)` oder `data(cars)`. Eine Beschreibung des
Datensatzes findet sich wie üblich in den Hilfedateien, z.B. `?iris`, `?cars`.
